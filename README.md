...
A colorful, icon-rich directory tree printer for your terminal. It walks a path, shows folders/files with Nerd 
Font icons, colors directories blue and files green, and can hide dotfiles by default.
...
- `-l`, `--level <n>` - limit the depth printed (0 keeps going).
- `--prune` - drop empty directories from the output.
- `-s`, `--size` - show file sizes in a human-readable form.
